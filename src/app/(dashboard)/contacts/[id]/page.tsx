...
export default async function ContactDetailPage({
  ...
  const contact = await prisma.contact.findUnique({
    where: { id },
    include: { deals: true, tasks: true, activities: true },
  });
    ...
    <div>
      <div className="md:flex md:items-center md:justify-between mb-8">
        <div className="flex-1 min-w-0">
          ...
          <p className="mt-1 text-sm text-gray-500">{contact.position} at {contact.company}</p>
        </div>
        <div className="mt-4 flex md:mt-0 md:ml-4 space-x-3">
          <button
            onClick={() => (document.getElementById("email-modal") as HTMLElement).style.display = "block"}
      ...
      {contact.email && (
        <div id="email-modal" style={{ display: "none" }}>
          <EmailCompose
            to={contact.email}
            onClose={() => (document.getElementById("email-modal") as HTMLElement).style.display = "none"}
          />
        </div>
      )}
    </div>
